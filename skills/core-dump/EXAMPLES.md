...
```json
{
  ...
  "emitted": ["<absolute path of every file this run wrote; renders as copyable file rows above the seal; every file listed here must ALSO be named inside the checkpoint itself>"],
  "resume_hint": "<OPTIONAL one short clause appended to the generated resume row ('/catchup at <path> and <this>'); omit when the checkpoint's Next action already says it>",
