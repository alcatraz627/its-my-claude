...
- `codex` is a valid model name in `--model-mix` (e.g. `--model-mix "opus=2,codex=1;jester=sonnet"`) and for the jester. A Codex voter is dispatched through the `codex:codex-rescue` agent with `--wait`, its prompt written to `02-voter-prompts/voter-N.md` and its proposal read back from `03-voter-proposals/voter-N.md` like any other seat; `voter_evidence` records `"model":"codex"` and the slice. A different model family is a lens, never a substitute for a distinct evidence slice, so a Codex voter still gets its own slice (D1). Worked example: `assets/magi/20260911-0240-clanky-character` (voter-3, high effort). The jester is the natural seat when only one foreign model is wanted, because the design already gives it a different model and opposite web access. Codex seats count against the Codex usage gate (`bash ~/.claude/adapters/codex/bin/codex-gcc gate`); a GATED verdict means seat that voter on a Claude model instead.
