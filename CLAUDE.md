...
| `conventions/callout-boxes.md` | `topic:callout-boxes`, `topic:hook-output`, `phrase:"pretty box"`, `tool:hook_box`, composing any inline callout | One anatomy for every inline callout box (hook/gate/subagent/ipc): closed glyph set, the → action line, width 72, dedup at source; box only when action is owed, else line-tag |
