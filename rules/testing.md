...
- `[declared-ready]` and `[collect-not-run]` — both live in full in [`rules/exercise-based-verification.md`](exercise-based-verification.md) (run the path before "done"; a collect, compile, or lint executes no assertion). Mechanically gated by the declared-ready Stop hook.
