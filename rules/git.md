...
One approval ≠ blanket approval. Each push to `main` or `master` requires fresh confirmation: guard-git-push.sh blocks and prints a nonce; print the owner the line `approve push <nonce>` once and keep working. He types it back as an ordinary message from any client and push-approve-prompt.sh (UserPromptSubmit) writes the single-use sentinel; every later message nags you with the line until he approves or types `cancel push`. Never call AskUserQuestion for a push: it halts the turn (owner ruling 2026-09-08). The `! touch` fallback works only in his own terminal shell. Protected repos gate ALL pushes the same way; unprotected feature-branch pushes flow freely.
