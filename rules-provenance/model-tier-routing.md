...
trivial lookup → `q`/haiku · mechanical sweep → sonnet-low or `lm fleet -m sweep` (granite, local, judge-gated) ·
...
UI-structure verify (which control is selected, counts, states) → `see --mlx` (Qwen3-VL, local, $0) ·
...
- The missing-pin warn path was retired 2026-09-18 (owner D6a): it had been muted
  for 37 days because the need had passed. The routing in this rule binds as text;
  the two fable hard blocks in guard-model-tier.sh stay. Usage-window advisories
  for fable are the policy layer's job (features/usage-policy.md). The
