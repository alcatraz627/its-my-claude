...
| `rules/cron-calendar-companion.md` | Every cron (launchd/crontab/CronCreate) gets a companion `Automations` calendar event for human visibility; retire both together | `topic:cron`, `topic:launchd`, `tool:CronCreate` |
...
| `scripts/claude-resilient.sh` | CLI wrapper (not a hook) | Wraps `claude` with exponential-backoff retry on API/network errors. Uses `claude --continue` for retries, so session state is preserved. Skips retry on 130/143 (user quit). Env: `CLAUDE_RESILIENT_MAX_RETRIES` (default 3), `BASE_SLEEP` (5s), `BACKOFF` (3), `DISABLED=1` | Handling API flakes / rate limits in long-running claude-CLI invocations; alias/symlink instead of raw `claude` |
...
| `scripts/schedule/schedule.sh` | CLI tool | gcc-schedule v0.1: `add`/`list`/`rm` for one-shot launchd LaunchAgents. Materialises script + plist + `Automations` Calendar event + bootstrap atomically. Per-name dir at `~/.claude/scheduled/<name>/`. Registry at `~/.claude/scheduled/registry.json`. Namespace = `com.alcatraz.<name>`. Aliased to `gcc-schedule` in `~/.zshrc` | Scheduling any launchd one-shot (Ghostty wake, reminder fire, deferred command); retiring an existing schedule cleanly; auditing what's loaded |
