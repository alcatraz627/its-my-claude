...
| `features/codex-adapter.md` | Router that lets OpenAI Codex CLI work under this gcc without inlining the rule corpus: generates `$CODEX_HOME/AGENTS.md` from a preamble plus `rules/00-index.md`, plus one PreToolUse guard for the three irreversible verbs | `topic:codex`, `tool:codex`, `topic:adapters` |
