...
| `~/.claude/assets/reports/20260704-gcc-structure-map/MAP.md` | **Living GCC structure map** (v3, `/gcc-map --deep`): how content reaches an agent, the doc graph, ranked empirical-vs-claimed divergences. Its TL;DR carries the open high-severity findings — they are live items, not archive | Any structural question about `~/.claude`; before restructuring config; checking what the map already flagged |
