...
| **the machine's token where a person's words belong** | The display defect class: a value crosses from a machine to a screen still written in the machine's vocabulary, because nothing translated it. Four shapes: a code (`LOW_CONFIDENCE`), a raw scalar (`0.6523`), a value's type (`[object Object]`), a sentinel rendered as a name (`unconfigured-module` as a heading). Fixing how such a token *wraps* rather than whether it belongs on screen is the same error one level down. | Reviewing any surface that renders values it did not author. Canonical: `rules/machine-token-where-human-words-belong.md`. Coined by the owner 2026-08-25 |
...
| **seamless** | A boundary that never shows up as a wall or a cost to the owner: a flow (across tools, agents, services, platforms, or input modes) completes end to end without him bridging a gap by hand, re-explaining, or cycling through a menu. Demands: the whole flow runs in one mode, state and intent cross the boundary intact, the code bends to the product rather than the reverse. Opposites he names: walls between services, hacky one-offs that make a module rigid, checklist goals instead of behavioural ones, breaking flow to switch modality. | 8 genuine occ 2026-07 to 2026-09 (features composing after a rework, keyboard-only kanban, /goal statements, the gcp umbilical cord, csync on the Pi, the Codex hands seat). For a Codex seat the five conditions are in `features/codex-adapter.md`. Baked 2026-09-10 |
